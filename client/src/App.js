// novelai-clone/client/src/App.js
...
export default function App() {
  ...
  const handleGenerate = async () => {
    ...
    setIsLoading(true);
    try {
      // Sending a default length of 1000 tokens for generation
      const text = await generateStory({ prompt, length: 1000 });
      setOutput(text);
    } catch (err) {
      console.error("Frontend caught error:", err); // Log the full error
      setError(err); // The error message from api.js
    } finally {
      setIsLoading(false);
    }
  };
